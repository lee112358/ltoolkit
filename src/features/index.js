...
export const FEATURES = [
	...
	},
	{
		...
		group: "editor",
		name: "Option 点击在侧栏打开",
		desc: "按住 Option 点链接、文件树、搜索结果或 Bases 表格里的文件名，总是在第二个标签组里正显示着的那个标签页打开 —— 左边清单、右边跟着换，整个窗口始终只有两个标签组。Obsidian 自带的 Option+Cmd+点击是「新建分栏」，点几次就多几个标签组；官方要做到复用得先手动分栏再把左边那个标签页固定住，固定之后左边自己就跳不动了。这个功能把那套行为收进一个修饰键，不用固定任何标签页。右边开了好几个标签页也没关系，看的是哪个就开在哪个里；只有一个标签组时才劈一刀。单按 Option 在 Obsidian 里本来不做任何事，不抢别的快捷键；编辑器正文里的 Option+点击多光标也不受影响——只有点在链接和文件项上才走这条路。文件树原本的 Option+点击多选会被这个功能占掉，Shift 连选不受影响。",
	...
	},
	{
		...
		group: "editor",
		name: "同一标签组不重复打开",
		desc: "已经有标签页开着这个文件时直接切过去，而不是再开一份 —— 拦在打开之前，文件压根不会进第二个标签页（为这次打开新建的空标签页顺手收掉）。搜索结果的高亮、链接里的小标题跟着一起切过去。只在同一标签组内生效，左右分栏对照看同一篇不受影响。",
